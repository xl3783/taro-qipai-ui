...
export default function TransferModal({ isOpen, onClose, players, currentPlayer, onTransfer }) {
  ...
  // 如果没有数据则不渲染
  if (!isOpen || !players || !currentPlayer) return null
      ...
      <View className='bg-white rounded-lg p-6 w-96'>
        ...
        <View className='space-y-6 py-4'>
          <View className='space-y-2'>
            <Text>付款人</Text>
            <Picker mode='selector' range={players?.map(p => p.name) || []} onChange={e => setFromPlayerId(players[e.detail.value]?.id || '')}>
              <View className='border p-2 rounded'>{fromPlayer?.name || '选择付款人'}</View>
            </Picker>
          </View>
          <View className='flex items-center justify-center my-2'>
            <Text>{fromPlayer?.name || '付款人'}</Text>
            <Text className='mx-2'>→</Text>
            <Text>{toPlayer?.name || '收款人'}</Text>
          </View>
          <View className='space-y-2'>
            <Text>收款人</Text>
            <Picker mode='selector' range={players?.filter(p => p.id !== fromPlayerId).map(p => p.name) || []} onChange={e => setToPlayerId(players?.filter(p => p.id !== fromPlayerId)[e.detail.value]?.id || '')}>
              <View className='border p-2 rounded'>{toPlayer?.name || '选择收款人'}</View>
            </Picker>
          </View>
          <View className='space-y-2'>
            <Text>转账金额</Text>
          ...
          <View className='grid grid-cols-2 gap-3 mt-4'>
            ...
            <Button onClick={handleTransfer} disabled={!fromPlayerId || !toPlayerId || !amount || fromPlayerId === toPlayerId} className='bg-orange-500 hover:bg-orange-600'>确认转账</Button>
          </View>
        </View>
      </View>
    </View>
